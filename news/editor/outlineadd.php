...
<?php 
    if($_POST[submit]){
    	...
    	$query2=mysql_query("insert into outline(sumid,resource,keywords,content) values('$sumid','$resource','$keywords','$content')");
        if($query1 and $query2){
    		echo "<script>alert('大纲添加成功!');history.go(-1);</script>";
    	}
    	else{
    		echo "<script>alert('大纲添加失败!');history.go(-1);</script>";
    	}
    }
